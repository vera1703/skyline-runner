...
@export var coin_height_min: float = 40.0   # Minimale Höhe über der Plattform
@export var coin_height_max: float = 120.0  # Maximale Höhe über der Plattform
...
func _spawn_coins_on_building(building: Node2D, platform_center_x: float, platform_y: float, platform_width: float) -> void:
	...
	for i in range(coin_count):
		...
		# und fügen dann den randomisierten Offset hinzu, um den Coin über dem Gebäude zu platzieren
		var coin_y := platform_y - (building_height * 0.5) - random_height_offset
		
		coin.position = Vector2(coin_x, coin_y)
		
		if debug_enabled:
			print("[DEBUG] Coin ", i + 1, " von ", coin_count, " gespawnt bei x=", coin_x, ", y=", coin_y, " (Höhenoffset: ", random_height_offset, ")")
